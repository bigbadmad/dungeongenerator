...
export function turnLeft(dir: Direction): Direction {
  const map: Record<Direction, Direction> = { north: 'west', west: 'south', south: 'east', east: 'north' };
  return map[dir];
}

export function turnRight(dir: Direction): Direction {
  const map: Record<Direction, Direction> = { north: 'east', east: 'south', south: 'west', west: 'north' };
  return map[dir];
}
...
function generateRoom(
  ...
  const room: Room = {
    ...
    rect,
    contents: contentsEntry.value as RoomContentsType,
    trap,
    dressing,
    exits,
    notes: '',
...
export function applyOverride(
  ...
  return {
    ...state,
    rollHistory: state.rollHistory.map(r =>
      r.id === rollId ? { ...r, overridden: true, overrideValue: value } : r,
    ),
  };
}

export function buildRoomDescription(room: Room): string {
  const shapeLabel: Record<string, string> = {
    square: 'Square', rectangle: 'Rectangular', circle: 'Circular', irregular: 'Irregular',
  };
  const contentsLabel: Record<string, string> = {
  ...
  if (room.trap) desc += ` (${room.trap.replace(/_/g, ' ')})`;
  if (room.dressing) desc += `. ${room.dressing.flavour}`;
  return desc;
}
